...
func TestBatchParseMultiField(t *testing.T) {
	...
	t.Run("MsgMhfRegisterEvent", func(t *testing.T) {
		...
		bf.WriteUint8(6)  // Unk4
		bf.WriteUint16(7) // Unk5
		bf.Seek(0, io.SeekStart)
		pkt := &MsgMhfRegisterEvent{}
		if err := pkt.Parse(bf, ctx); err != nil {
			t.Fatal(err)
		}
		if pkt.AckHandle != 1 || pkt.Unk0 != 2 || pkt.Unk5 != 7 {
			t.Error("field mismatch")
		}
	})
	...
	t.Run("MsgMhfUpdateCafepoint", func(t *testing.T) {
		...
		if pkt.AckHandle != 1 || pkt.Unk0 != 2 || pkt.Unk1 != 3 {
			t.Error("field mismatch")
		}
	})
	...
	t.Run("MsgMhfAcquireTitle", func(t *testing.T) {
		...
		if pkt.TitleID != 4 {
			t.Errorf("TitleID = %d, want 4", pkt.TitleID)
		}
	})
	...
	t.Run("MsgSysHideClient", func(t *testing.T) {
		...
		if !pkt.Hide || pkt.Unk0 != 1 || pkt.Unk1 != 2 {
			t.Error("field mismatch")
		}
	})
	...
	t.Run("MsgMhfOprMember", func(t *testing.T) {
		...
		if !pkt.Blacklist || pkt.Operation || pkt.CharID != 99 {
			t.Error("field mismatch")
		}
	})
...
func TestBatchParseVariableLength(t *testing.T) {
	...
	t.Run("MsgSysRecordLog", func(t *testing.T) {
		...
		if len(pkt.DataBuf) != 4 {
			t.Errorf("DataBuf len = %d, want 4", len(pkt.DataBuf))
		}
	})
...
func TestBatchParseSysLoadRegister(t *testing.T) {
	...
	if pkt.RegisterID != 2 || pkt.Unk1 != 3 {
		t.Error("field mismatch")
	}
}

// TestBatchParseSysLoadRegisterError tests validation error in SysLoadRegister.
func TestBatchParseSysLoadRegisterError(t *testing.T) {
	bf := byteframe.NewByteFrame()
	bf.WriteUint32(1) // AckHandle
	bf.WriteUint32(2) // RegisterID
	bf.WriteUint8(3)  // Unk1
	bf.WriteUint16(1) // fixedZero0 = 1 (NOT zero - should error)
	bf.WriteUint8(0)  // fixedZero1
	bf.Seek(0, io.SeekStart)
	...
	if err == nil {
		t.Error("expected error for non-zero fixed values")
	}
}
...
// TestBatchParseSysOperateRegisterError tests validation error.
func TestBatchParseSysOperateRegisterError(t *testing.T) {
	bf := byteframe.NewByteFrame()
	bf.WriteUint32(1) // AckHandle
	bf.WriteUint32(2) // SemaphoreID
	bf.WriteUint16(1) // fixedZero = 1 (NOT zero - should error)
	...
	err := pkt.Parse(bf, &clientctx.ClientContext{})
	if err == nil {
		t.Error("expected error for non-zero fixed value")
	}
}
...
func TestBatchParseNotImplemented(t *testing.T) {
	...
	packets := []MHFPacket{
		...
		&MsgMhfEnumerateItem{}, &MsgMhfAcquireItem{},
		&MsgMhfUpdateGuildcard{},
		&MsgMhfGetCogInfo{}, &MsgMhfGetNotice{}, &MsgMhfPostNotice{},
		&MsgCaExchangeItem{},
	}
...
func TestBatchParseAnnounce(t *testing.T) {
	...
	if pkt.IPAddress != 0x7F000001 || pkt.Port != 54001 || pkt.Type != 1 {
		t.Error("field mismatch")
	}
}
