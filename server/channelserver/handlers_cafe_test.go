...
func TestHandleMsgMhfPostBoostTime(t *testing.T) {
	...
	// Empty handler - should not panic
	defer func() {
		if r := recover(); r != nil {
			t.Errorf("handleMsgMhfPostBoostTime panicked: %v", r)
		}
	}()

	handleMsgMhfPostBoostTime(session, nil)
}
...
func TestHandleMsgMhfPostBoostTimeLimit(t *testing.T) {
	...
	defer func() {
		if r := recover(); r != nil {
			t.Errorf("handleMsgMhfPostBoostTimeLimit panicked: %v", r)
		}
	}()

	handleMsgMhfPostBoostTimeLimit(session, nil)
}
