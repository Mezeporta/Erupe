...
func handleMsgMhfEnumerateDistItem(s *Session, p mhfpacket.MHFPacket) {
	...
	for _, dist := range itemDists {
		...
		bf.WriteUint8(0)  // Unk (G7+)
		bf.WriteUint16(0) // Unk (G6+)
		if dist.Selection {
			bf.WriteUint8(2) // Selection (G8+)
		} else {
			bf.WriteUint8(0)
		}
		bf.WriteUint16(0) // Unk (G7+)
		bf.WriteUint16(0) // Unk (G7+)
		bf.WriteUint8(0)  // Unk (G10+)
		ps.Uint8(bf, dist.EventName, true)
		// 6 item slots with 13 entries each (G8+ format)
		for i := 0; i < 6; i++ {
			for j := 0; j < 13; j++ {
				bf.WriteUint8(0)
				bf.WriteUint32(0)
...
func handleMsgMhfApplyDistItem(s *Session, p mhfpacket.MHFPacket) {
	...
	for _, item := range distItems {
		...
		bf.WriteUint32(item.ID) // G8+ includes item ID
	}
	doAckBufSucceed(s, pkt.AckHandle, bf.Data())
}
